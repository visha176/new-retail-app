...
def aggregate_data(df, threshold_date):
    ...
    return df.groupby(['City', 'UPC/Barcode/SKU', 'STORE_NAME', 'DESIGN', 'Adjusted 1st Rcv Date', 'Class', 'SubClass', 'Size', 'Color']).agg({
        'Shop Rcv Qty': 'sum',
        'Disp. Qty': 'sum',
        'O.H Qty': 'sum',
        'Sold Qty': 'sum'
...
def calculate_design_sell_through(df):
    ...
    design_totals = df.groupby(['UPC/Barcode/SKU', 'City']).agg({'Sold Qty': 'sum', 'Net Receiving': 'sum'}).reset_index()
...
def merge_data(desired_df, design_totals):
    return pd.merge(desired_df, design_totals[['UPC/Barcode/SKU', 'City', 'design Sell Through']], on=['UPC/Barcode/SKU', 'City'], how='left')
...
def process_data(desired_df):
    ...
    merged_df = pd.merge(desired_df, article_days, on=['UPC/Barcode/SKU', 'City'], how='left', suffixes=('', '_max_days'))
    merged_df_grouped = merged_df.groupby(['UPC/Barcode/SKU', 'City']).agg({
        'O.H Qty': 'sum',
        'Sold Qty': 'sum',
        'Shop Days': 'max'
    }).reset_index()
    result_df = merged_df_grouped[['UPC/Barcode/SKU', 'City', 'Shop Days']].rename(columns={'Shop Days': 'Date Difference'})
    return result_df

def process_and_calculate_cover(df, article_days):
    merged_df = pd.merge(df, article_days, on=['UPC/Barcode/SKU', 'City'], how='left', suffixes=('', '_max_days'))
    merged_df_grouped = merged_df.groupby(['UPC/Barcode/SKU', 'City']).agg({
        'O.H Qty': 'sum',
        'Sold Qty': 'sum',
        'Shop Days': 'max'
    ...
    result_df = merged_df_grouped[['UPC/Barcode/SKU', 'City', 'Shop Days']].rename(columns={'Shop Days': 'Date Difference'})
    merged_df_grouped = pd.merge(merged_df_grouped, result_df, on=['UPC/Barcode/SKU', 'City'], how='left')
...
def merge_with_desired_cover(desired_df, merged_df_grouped):
    desired_df = pd.merge(desired_df, merged_df_grouped[['UPC/Barcode/SKU', 'City', 'Targeted Cover']], on=['UPC/Barcode/SKU', 'City'], how='left')
...
def merge_desired_with_article_days(desired_df, article_days):
    desired_df = pd.merge(desired_df, article_days, on=['UPC/Barcode/SKU', 'City'], how='left')
    return desired_df
    ...
    for sending_index, sending_row in sending_stores.iterrows():
        ...
        for receiving_index, receiving_row in matches.iterrows():
            ...
            transfer_details.append({
                'City': sending_row['City'],  # Changed Zone to City
